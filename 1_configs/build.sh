...
do_build() {
    ...
    for _f in gitconfig gitattributes gitignore; do
        [ -f "$DIST_DIR/git/$_f" ] && mv "$DIST_DIR/git/$_f" "$DIST_DIR/$_f"
    done
    [ -f "$DIST_DIR/git/gitmodules" ] && { mkdir -p "$DIST_DIR/modules"; mv "$DIST_DIR/git/gitmodules" "$DIST_DIR/modules/gitmodules"; }
...
# Plus root_targets for single files at the repo root (.mcp.json). Never
# purge-then-copy: .claude/ and .obsidian/ mix managed config with per-machine
# state.
do_dotfiles() {
